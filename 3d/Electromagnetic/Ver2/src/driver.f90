...
DO i_sim = 1, maxsteps
    ...
    ! evolve ring current_type
    CALL ring_current(i_sim)
...
PRINT*,'Time Elapsed (seconds) = ',t2-t1
PRINT*,''
...
SUBROUTINE ring_current(tStep)

    INTEGER, INTENT(IN) :: tStep
    REAL*8 :: x,y,z,r,Rc,curr, currmax
    INTEGER :: i,j,k,endStep
    
    ! clear current density array
    Jx = 0.d0
    Jy = 0.d0
    Jz = 0.d0
    
    endStep = 100 ! stop increasing current after endStep time steps
    
    ! radius of current loop
    Rc = DBLE(nx/8)
    
    ! linearly increasing current
    currmax = 0.005*endStep*dt
    
    IF(tStep .LT. endStep) THEN
        curr = currmax*DBLE(tStep)/DBLE(endstep) 
    ELSE
        curr = currmax
    END IF
    ...
    IF(fields_out) THEN
       ...
       !OPEN(UNIT=12,FILE=filename2)
        ...
        !END DO
         ...
         DO j = 1, nz
            DO i = 1, nx
                WRITE(11,*) i*dx,j*dz,Bx_grid(i,ny/2,j),Bz_grid(i,ny/2,j) 
                !WRITE(11,*) i*dx,j*dy,Jx(i,j,nz/2),Jy(i,j,nz/2) 
            END DO
        END DO
