    ...
    DO i= 1,ns   
        ...
        DO j = 1, Np_in(i)
            ...
            z = particles(i,j)%z
                ...
                IF(i .EQ. 1) THEN
                    particles(i,j)%uz =  (5.d0*twopi/100.d0)*COS(twopi*ts/100.d0)
                    particles(i,j)%z =    (nz-8) + 5.d0*SIN(twopi*ts/100.d0)
                ELSE IF(i .EQ. 2) THEN
                    particles(i,j)%uz = -(5.d0*twopi/200.d0)*COS(twopi*ts/100.d0)
                    particles(i,j)%z = (nz-8) - 5.d0*SIN(twopi*ts/100.d0)
                END IF
...
SUBROUTINE deposit_currents_zigzag()
    ...
    REAL*8  :: idx, idy, idz, idxyz, igam, J_max         
...
SUBROUTINE particle_boundary()
    ...
    bparticles_xm(0) = 0.d0
    bparticles_xp(0) = 0.d0
    bparticles_ym(0) = 0.d0
    bparticles_yp(0) = 0.d0
...
! TO DO: Keep track of particles escaping into corner and edge domains. Make 12 more bparticle_corner/edge arrays.
...
SUBROUTINE delete_particle(i,j)
    ...
    IF(Np_esc_xm .GE. max_particles .OR. Np_esc_xp .GE. max_particles .OR. &
       ...
       Np_esc_zm .GE. max_particles .OR. Np_esc_zp .GE. max_particles ) THEN
    ...
    ELSE IF(temp(2) .GT. ymax)THEN
        ...
        bparticles_yp(ix+5) = temp(6)

    END IF
    
    
    ! Need to also kep track of particles escaping into corner and edge domains (20 total)
    
    
    ! Dont need to worry about z boundary
