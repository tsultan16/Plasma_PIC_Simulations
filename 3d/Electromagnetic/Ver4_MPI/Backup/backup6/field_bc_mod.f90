    ...
    IF(mycoord(1)  .EQ. nranks_x-1) THEN
        ...
        DO j = 1, ny
            DO k = 1, nz
                ...
                temp=.25*s*ey(nx,j,k)
                ey(nx-1,j,k)=ey(nx-1,j,k)-temp
                ey(nx+1,j,k)=ey(nx+1,j,k)-temp
                ey(nx,j,k)=ey(nx,j,k)-2.*temp
      
                temp=.25*s*ez(nx,j,k)
                ez(nx-1,j,k)=ez(nx-1,j,k)-temp
                ez(nx+1,j,k)=ez(nx+1,j,k)-temp
                ez(nx,j,k)=ez(nx,j,k)-2.*temp
    ...
    IF(ndims .EQ. 2) THEN
        ...
        DO k = 1, nz
            DO i = 1, nx
                ...
                temp=.25*s*ez(i,ny,k)
                ez(i,ny-1,k)=ez(i,ny-1,k)-temp
                ez(i,ny+1,k)=ez(i,ny+1,k)-temp
                ez(i,ny,k)=ez(i,ny,k)-2.*temp
                
                temp=.25*s*ex(i,ny,k)
                ex(i,ny-1,k)=ex(i,ny-1,k)-temp
                ex(i,ny+1,k)=ex(i,ny+1,k)-temp
                ex(i,ny,k)=ex(i,ny,k)-2.*temp
...
SUBROUTINE boundary_conduct_z(s)
    ...
    DO i = 1, nx
            DO j = 1, ny
                ...
                temp=.25*s*ex(i,j,ny)
                ex(i,j,ny-1)=ex(i,j,ny-1)-temp
                ex(i,j,ny+1)=ex(i,j,ny+1)-temp
                ex(i,j,ny)=ex(i,j,ny)-2.*temp
                
                temp=.25*s*ey(i,j,ny)
                ey(i,j,ny-1)=ey(i,j,ny-1)-temp
                ey(i,j,ny+1)=ey(i,j,ny+1)-temp
                ey(i,j,ny)=ey(i,j,ny)-2.*temp
