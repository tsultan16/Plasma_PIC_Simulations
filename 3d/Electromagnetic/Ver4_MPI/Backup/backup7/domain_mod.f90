...
SUBROUTINE exchange_B(layers)
    ...
    CALL exchange_fields(Bx, By, Bz, layers)
...
SUBROUTINE exchange_E(layers)
    ...
    CALL exchange_fields(Ex, Ey, Ez, layers)  
...
SUBROUTINE exchange_current(layers)
    ...
    CALL exchange_fields(Jx, Jy, Jz, layers)  
...
SUBROUTINE exchange_fields(fx, fy, fz, layers)
    ...
    CALL MPI_WIN_FENCE(0, win, ierr)
        ...
        CALL field_packup(field_buffer_xm, 1, fx, fy, fz, layers)
        ...
        CALL field_packup(field_buffer_xp, 2, fx, fy, fz, layers)        
        ...
        IF(ndims .EQ. 2) THEN
            ...
            CALL field_packup(field_buffer_ym, 3, fx, fy, fz, layers)
            ...
            CALL field_packup(field_buffer_yp, 4, fx, fy, fz, layers)
    ...
    CALL field_unpack(fx, fy, fz, layers)    
...
SUBROUTINE field_packup(buffer_out, bndry, fx, fy, fz, layers)
...
SUBROUTINE field_unpack(fx, fy, fz, layers)
    ...
    ! For E and B field arrays (layers=1), make sure that the boundary ghost cells have been cleared out before this subroutine is called.
    ...
    DO k = -1, nz+3
        DO j = -1, ny+3         
            DO i = 1, layers                    
               ...
               fx(i-1,j,k) = fx(i-1,j,k) + mem_fields(ix)       
               fy(i-1,j,k) = fy(i-1,j,k) + mem_fields(ix+1)    
               fz(i-1,j,k) = fz(i-1,j,k) + mem_fields(ix+2)     
               ix = ix + 3
        ...
        DO k = -1, nz+3
            DO j = 1, layers
                DO i = -1, nx+3         
                    ...
                    fx(i,j-1,k) = fx(i,j-1,k) + mem_fields(ix)    
                    fy(i,j-1,k) = fy(i,j-1,k) + mem_fields(ix+1)   
                    fz(i,j-1,k) = fz(i,j-1,k) + mem_fields(ix+2)    
                    ix = ix + 3
