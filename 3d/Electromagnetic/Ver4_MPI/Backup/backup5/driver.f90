...
SUBROUTINE solve()
    ...
    ! add ring current
    IF(myrank .EQ. 1) CALL add_ring_current(i_sim)
...
SUBROUTINE output_bin(ts)
    ...
    filename1 = trim('Frames/Snapshots/Bvector_xz_rank=')//TRIM(mrank_x)//TRIM('_t=')//TRIM(uniti)//TRIM('.dat')
    ...
    filename_e_xz = trim('Frames/Snapshots/electrons_xz_rank=')//TRIM(mrank_x)//TRIM('_t=')//TRIM(uniti)//TRIM('.dat')
    filename_i_xz = trim('Frames/Snapshots/ions_xz_rank=')//TRIM(mrank_x)//TRIM('_t=')//TRIM(uniti)//TRIM('.dat')
    
    !filename_cur = trim('Frames/Snapshots/Jvector_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_cur = trim('Frames/Snapshots/Jvector_xz_rank=')//TRIM(mrank_x)//TRIM('_t=')//TRIM(uniti)//TRIM('.dat')
         ...
         DO j = 1, nz
            DO i = 1, nx
                ...
                IF(MOD(j,1) .EQ. 0 .AND. MOD(i,1) .EQ. 0) THEN
                   ...
                   !IF(Bmag .LT. 0.01) THEN
                        WRITE(11)  i*dx+mycoord(1)*Lx ,j*dz,Bx_grid(i,ny/2,j)/c,Bz_grid(i,ny/2,j)/c 
                    !ELSE
    ...
    IF(particles_out) THEN
        ...
        DO i = 1, Np_in(1)
            ...
            IF( ABS(particles(1,i)%y -ny/2) .LT. 2) WRITE(9) particles(1,i)%x+(mycoord(1))*Lx,particles(1,i)%z            
            IF( ABS(particles(1,i)%z -nz/2) .LT. 2) WRITE(7) particles(1,i)%x+(mycoord(1))*Lx,particles(1,i)%y+(mycoord(2))*Ly
        END DO
        ...
        IF(ns .EQ. 2) THEN       
            DO i = 1, Np_in(2) 
                ...
                IF( ABS(particles(2,i)%y -ny/2) .LT. 2) WRITE(10) particles(2,i)%x+(mycoord(1))*Lx,particles(2,i)%z
                IF( ABS(particles(2,i)%z -nz/2) .LT. 2) WRITE(8) particles(2,i)%x+(mycoord(1))*Lx,particles(2,i)%y+(mycoord(2))*Ly
            END DO
        END IF
