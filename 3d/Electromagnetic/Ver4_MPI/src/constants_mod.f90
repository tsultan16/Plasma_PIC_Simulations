...
INTEGER, PARAMETER :: nx       = 200    	     ! mesh size (use only powers of 2 to avoid malloc memory corruption) 
INTEGER, PARAMETER :: ny       = 200
INTEGER, PARAMETER :: nz       = 1
INTEGER, PARAMETER :: nb       = 3
INTEGER, PARAMETER :: ns       = 2     	  		 ! number of particle species
INTEGER, PARAMETER :: Np_max   = 1048576/2  	     ! max number of particles per species

INTEGER, PARAMETER :: maxsteps = 20             ! max. number of time steps
INTEGER, PARAMETER :: tskip    = 10              ! file output frequency

INTEGER, PARAMETER :: interpolation_type = 2     ! 1: zeroth order (NGP), 2: first order (linear/CIC), 3: second-order (quadtratic spline)
INTEGER, PARAMETER :: bndry = 2                  ! 1: periodic (both x,y) , 2: open (i.e. radiation absorbing and particle outflow), 3:reflecting 
INTEGER, PARAMETER :: current_type = 1           ! 1: Zig-zag scheme 2: Esirkepov's scheme 3: Villasenor-Buneman scheme (2 and 3 are unstable when particle number becomes large...need to figure out why)
INTEGER, PARAMETER :: N_filter = 2               ! how many times to repeat current filter (1 seems to be good enough for coarser grids)
...
REAL*8, PARAMETER :: c    = 0.45d0       ! speed of light in vacuum (also serves as the Courant number, for stability set this to be slightly <0.5)
REAL*8, PARAMETER :: q_e  = -1.d-4!-1.d-4        ! electron charge
REAL*8, PARAMETER :: q_i  = -q_e        ! ion charge
REAL*8, PARAMETER :: m_e = 1.d0*q_i         ! charge to mass ratio - electrons
REAL*8, PARAMETER :: m_i = 16.d0*m_e        ! charge to mass ratio - ions
...
LOGICAL, PARAMETER :: file_restart = .FALSE.
...
REAL*8, PARAMETER  :: v0x   = 0.1d0!0.25d0/1.0     ! intial drift velocity
REAL*8, PARAMETER  :: v0y   = 0.d0 
REAL*8, PARAMETER  :: v0z   = 0.0d0 
REAL*8, PARAMETER  :: vth_e  = 0.1d0/1.0       ! R.M.S. thermal velocity
REAL*8, PARAMETER  :: vth_i  = 0.025d0/1.0     ! R.M.S. thermal velocity
...
LOGICAL, PARAMETER :: particles_out = .FALSE. 
...
INTEGER, PARAMETER :: max_particles = Np_max/8 ! maximum number of particles allowed per MPI send
