...
DO i_sim = tstart, tstart+maxsteps-1
	...
	B_max = MAX(B_max,MAXVAL(By_grid(:,je-4,ke)))
...
PRINT*,'B_max, maxloc = ',(B_max), MAXLOC(By(:,je-4,ke))     
...
SUBROUTINE set_domain()
    ...
    IF(interpolation_type .EQ. 3 .AND. current_type .NE. 2) THEN
        PRINT*,'ERROR. Need to use current_type=2 for second order interpolation of charge and forces...'
        STOP
    END IF
...
SUBROUTINE solve()
    ...
    ! neutralize excess boundary charges (by spreading out lateral currents) 
    ...
    PRINT*,'Exchanging currents...'

    ! pre-filter exchange
    CALL exchange_current(2,0)  ! Filtering needs to be done after exchange. For multiple filtering cycles, boundary currents 
    ...
    DO k = 1 , N_filter      
        ...
        IF(k .LT. N_filter)  CALL exchange_current(1,0) ! only need one boundary layer for next filtering cycle     
...
SUBROUTINE output_bin(ts)
    ...
    filename3 = trim('Frames/Snapshots/Bz_t=')//TRIM(uniti)//TRIM('.dat')
    ...
    IF(fields_out) THEN
                ...
                Bmag = SQRT(Bmag)
                 IF(MOD(j,2) .EQ. 0 .AND. MOD(i,2) .EQ. 0) THEN
                  !   IF(Bmag .LT. 200.d0) THEN
                         WRITE(11) i*dx+mycoord(1)*nx ,j*dy+mycoord(2)*ny,Ex_grid(i,j,ke)/Bmag,Ey_grid(i,j,ke)/Bmag
                         !WRITE(11) i*dx+mycoord(1)*nx ,j*dz, Jx(i,ny/2,j),Jz(i,ny/2,j)
                   !  ELSE
        ...
        DO j = 1, ny
            DO i = 1, nx
                Bmag = Bx_grid(i,j,ke)**2 + By_grid(i,j,ke)**2 
                !Bmag = Jx(i,j,ke)**2 + Jy(i,j,ke)**2 
                Bmag = SQRT(Bmag)
                 IF(MOD(j,10) .EQ. 0 .AND. MOD(i,10) .EQ. 0) THEN
                  !   IF(Bmag .LT. 200.d0) THEN
                         WRITE(12) i*dx+mycoord(1)*nx ,j*dy+mycoord(2)*ny,Bx_grid(i,j,ke)/Bmag,By_grid(i,j,ke)/Bmag
                         !WRITE(12) i*dx+mycoord(1)*nx ,j*dy+mycoord(2)*ny, Jx(i,j,ke)/Bmag,Jy(i,j,ke)/Bmag
                   !  ELSE
                 !       WRITE(12) i*dx+mycoord(1)*Lx ,j*dy+mycoord(2)*Ly,0.d0,0.d0 
                  !   END IF
                 END IF      
        ...
        DO i = 1, nx
            WRITE(13) i*dx+mycoord(1)*nx , By_grid(i,je-4,ke)               
        END DO
