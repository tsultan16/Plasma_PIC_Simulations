...
SUBROUTINE compute_poynting_flux()
    ...
    DO j = 1, ny
        DO i = 1, nx
            ...
            S = S - (Ex_grid(i,j,1)*By_grid(i,j,1) - Ey_grid(i,j,1)*Bx_grid(i,j,1))
        END DO
    END DO    
   ...
   ! flux through y+, y- faces
    DO k = 1, nz
        DO i = 1, nx
            ...
            S = S - (Ez_grid(i,1,k)*Bx_grid(i,1,k) - Ex_grid(i,1,k)*Bz_grid(i,1,k))
        END DO
    END DO    
...
SUBROUTINE output_bin(ts)
    ...
    IF(fields_out) THEN
        ...
        !END DO
         ...
         DO j = 1, nz
            DO i = 1, nx
                ...
                WRITE(13) i*dx,j*dz,SQRT(Bx_grid(i,ny/2,j)**2+By_grid(i,ny/2,j)**2+Bz_grid(i,ny/2,j)**2)/c**2 
            END DO
        END DO
        ...
        DO i = 1, nz
            WRITE(12) DBLE(i)*dz,Bz(INT(0.6*nx),ny/2,i)/c 
        END DO
    ...
    IF(particles_out) THEN
        ...
        DO i = 1, N
            IF(.NOT. particles(1,i)%oob)THEN !.AND. INT(particles(1,i)%y) .EQ. ny/2) THEN
                WRITE(9) particles(1,i)%x,particles(1,i)%z
            ELSE   
                WRITE(9) -1.d0,-1.d0
            END IF            
                IF(ns .EQ. 2) THEN
                    IF(.NOT. particles(2,i)%oob)THEN ! .AND. INT(particles(2,i)%y) .EQ. ny/2) THEN
