...
SUBROUTINE init_particles()
        ...
        DO j = jmin, jmax
            DO i = imin, imax
                ...
                DO l = 1, N_cell	
                    ...
                    particles(1,counter)%x = i*dx  !- 0.1*dx/N_cell   
                    particles(1,counter)%y = j*dy  !+ l*dy/N_cell - 0.1*dy/N_cell   
                    particles(1,counter)%z = k*dz
               
                    ! set initial velocity : v(0) sampled uniformly from (vmin,vmax)
                    particles(1,counter)%ux = u0x !+ fv0(vmin+((vmax-vmin)/N_cell)*(l-0.5))
                    particles(1,counter)%uy = u0y !+ fv0(vmin+((vmax-vmin)/N_cell)*(l-0.5))
                    particles(1,counter)%uz = u0z
                
                    IF(ns .EQ. 2) THEN
                        particles(2,counter)%x = i*dx  !- 0.1*dx/N_cell   
                        particles(2,counter)%y = j*dy  !+ l*dy/N_cell - 0.1*dy/N_cell   
                        particles(2,counter)%z = k*dz
               
                        ! set initial velocity : v(0) sampled uniformly from (vmin,vmax)
                        particles(2,counter)%ux = u0x !+ fv0(vmin+((vmax-vmin)/N_cell)*(l-0.5))
                        particles(2,counter)%uy = u0y !+ fv0(vmin+((vmax-vmin)/N_cell)*(l-0.5))
                        particles(2,counter)%uz = u0z
                    END IF
    ...
    IF(ns .EQ. 2) THEN
        ...
        particles(2,1)%ux = gam*u0x 
        particles(2,1)%uy = gam*u0y 
        particles(2,1)%uz = gam*u0z
    END IF    
