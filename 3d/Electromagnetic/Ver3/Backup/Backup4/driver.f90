...
SUBROUTINE solve()
    ...
    IF(i_sim .GT. 45) CALL move_particles(i_sim)
    ...
    IF(i_sim .GT. 45) CALL particle_boundary()


    !*********************************
    ! Neutralize excess boundary charge
    !*********************************
    IF(i_sim .GT. 45) CALL neutralize_boundary_charge()
...
SUBROUTINE output_bin(ts)
    ...
    filename1 = trim('Frames/Snapshots2/Bvector_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename2 = trim('Frames/Snapshots2/Bvector_xy_t=')//TRIM(uniti)//TRIM('.dat')
    filename3 = trim('Frames/Snapshots2/density_e_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename4 = trim('Frames/Snapshots2/density_i_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_e_xz = trim('Frames/Snapshots2/electrons_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_i_xz = trim('Frames/Snapshots2/ions_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_e_yz = trim('Frames/Snapshots2/electrons_yz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_i_yz = trim('Frames/Snapshots2/ions_yz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_e_xy = trim('Frames/Snapshots2/electrons_xy_t=')//TRIM(uniti)//TRIM('.dat')
    filename_i_xy = trim('Frames/Snapshots2/ions_xy_t=')//TRIM(uniti)//TRIM('.dat')
    filename_cur = trim('Frames/Snapshots2/Jvector_xy_t=')//TRIM(uniti)//TRIM('.dat')
    ...
    IF(fields_out) THEN
         ...
         DO j = 1, nz
            DO i = 1, nx
                IF(MOD(j,1) .EQ. 0 .AND. MOD(i,1) .EQ. 0) THEN
                    ...
                    Bmag = SQRT(Bmag)
                    IF(Bmag .LT. 100) THEN
                        WRITE(11) i*dx,j*dz,Bx_grid(i,je,j)/c,Bz_grid(i,je,j)/c 
                    ELSE
                ...
                !WRITE(13) i*dx,j*dz,ne(i,ny/2,j)
        ...
        DO j = 1, ny
            DO i = 1, nx
                 IF(MOD(j,1) .EQ. 0 .AND. MOD(i,1) .EQ. 0) THEN
                     Bmag = (Bx_grid(i,j,nz/2)/c)**2 + (By_grid(i,j,nz/2)/c)**2 
                     Bmag = SQRT(Bmag)
                     IF(Bmag .LT. 100) THEN
                        WRITE(12) i*dx,j*dy,Bx_grid(i,j,nz/2)/c,By_grid(i,j,nz/2)/c
                     ELSE
                        WRITE(12) i*dx,j*dy,0.d0,0.d0 
                     END IF
                 END IF      
            END DO
        END DO
    ...
    IF(particles_out) THEN
        ...
        DO i = 1, Np_in(1)
            IF( ABS(particles(1,i)%y -ny/2) .LT. 1) WRITE(9) particles(1,i)%x,particles(1,i)%z
            IF( ABS(particles(1,i)%z -nz/2) .LT. 1) WRITE(7) particles(1,i)%x,particles(1,i)%y
        END DO
       
        IF(ns .EQ. 2) THEN       
            DO i = 1, Np_in(2) 
                IF( ABS(particles(2,i)%y -ny/2) .LT. 1) WRITE(10) particles(2,i)%x,particles(2,i)%z
                IF( ABS(particles(2,i)%z -nz/2) .LT. 1) WRITE(8) particles(2,i)%x,particles(2,i)%y
            END DO
        END IF
