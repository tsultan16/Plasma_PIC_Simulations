...
SUBROUTINE move_particles(ts)
    ...
    DO i = 1, ns
        IF(Np_in(i) .GT. 0) THEN
            pos_buffer(i,Np_in(i),1) = particles(i,Np_in(i))%x
            pos_buffer(i,Np_in(i),2) = particles(i,Np_in(i))%y
            pos_buffer(i,Np_in(i),3) = particles(i,Np_in(i))%z
        END IF    
    END DO
    ...
    DO i= 1,ns   
        ...
        DO j = 1, Np_in(i)
            ...
            !IF(.NOT. particles(i,j)%oob .AND. ts .GT. 62) THEN
...
SUBROUTINE particle_boundary()
    ...
    DO i= 1,ns 
        ...
        DO WHILE(counter .LE. Np_in(i))
            ...
            IF(bndry .EQ. 2) THEN 
                ...
                !PRINT*,'Species, counter, x, y, z = ',i,counter,particles(i,counter)%x,particles(i,counter)%y,particles(i,counter)%z
                ...
                IF( particles(i,counter)%x .LT. xmin .OR. particles(i,counter)%x .GT. xmax .OR. &
                    ...
                    Np_esc(i) = Np_esc(i) + 1 
                    !PRINT*,'Out of bound particle Species, Number = ',i,counter
    ...
    PRINT*,''
    PRINT*,'N_esc(electrons) = ',Np_esc(1)
    PRINT*,'N_esc(ions) = ',Np_esc(2)
    PRINT*,''
    Ntot_esc(1) = Ntot_esc(1) + Np_esc(1)
    Ntot_esc(2) = Ntot_esc(2) + Np_esc(2)
...
SUBROUTINE inject_particles()
    ...
    !*****************************************
    ! inject 25 particle pairs through x- face
    !*****************************************            
    DO j = 15, 75, 5 
        DO i = 15, 75, 5 
            ...
            CALL RANDOM_NUMBER(p)                                        
            p = p - 0.5
            uxe = v0x + vth_e*(vth_e + p(1) + p(2) + p(3) - 1.5d0)
                                        
            CALL RANDOM_NUMBER(p)                                       
            p = p - 0.5
            uye = vth_e*(p(1) + p(2) + p(3) - 1.5d0)

            CALL RANDOM_NUMBER(p)                                       
            p = p - 0.5                
            uze = vth_e*(p(1) + p(2) + p(3) - 1.5d0) 
            ...
            CALL RANDOM_NUMBER(p)                                      
            p = p - 0.5
            uxi = v0x + vth_i*(vth_i + p(1) + p(2) + p(3) - 1.5d0)
                                        
            CALL RANDOM_NUMBER(p)                                      
            p = p - 0.5
            uyi = vth_i*(p(1) + p(2) + p(3) - 1.5d0)

            CALL RANDOM_NUMBER(p)                    
            p = p - 0.5                  
            uzi = vth_i*(p(1) + p(2) + p(3) - 1.5d0)
    ...
    DO j = 15, 75, 5 
        DO i = 10, 130, 10
            ...
            CALL RANDOM_NUMBER(p)                                       
            p = p - 0.5                
            uze = vth_e*ABS((p(1) + p(2) + p(3) - 1.5d0)) 
    ...
    DO j = 15, 75, 10 
        DO i = 10, 130, 20
            ...
            CALL RANDOM_NUMBER(p)                    
            p = p - 0.5                  
            uzi = vth_i*ABS(p(1) + p(2) + p(3) - 1.5d0)
    ...
    DO j = 15, 75, 5 
        DO i = 10, 130, 10
            ...
            CALL RANDOM_NUMBER(p)                                       
            p = p - 0.5
            uye = vth_e*ABS(p(1) + p(2) + p(3) - 1.5d0)
    ...
    DO j = 15, 75, 10 
        DO i = 10, 130, 20
            ...
            CALL RANDOM_NUMBER(p)                                      
            p = p - 0.5
            uyi = vth_i*ABS(p(1) + p(2) + p(3) - 1.5d0)
...
SUBROUTINE ring_current()
    ...
    DO j = 1, ny
        DO i = 1, nx
            ...
            IF( ABS(r-Rc) .LT. 0.5d0) THEN
                Jx(i,j,nz/2) = curr * (-y/r)    ! right-handed current loop             
                Jy(i,j,nz/2) = curr * (x/r)                        
            END IF
    ...
    !PRINT*,'Jx_max = ',MAXVAL(Jx)
    !PRINT*,'Jy_max = ',MAXVAL(Jy)
    !PRINT*,'Jz_max = ',MAXVAL(Jz)
