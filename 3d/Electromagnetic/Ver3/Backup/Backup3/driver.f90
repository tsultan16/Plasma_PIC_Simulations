...
!IF(energy_out) OPEN(UNIT=18, FILE=file_energy, FORM = 'UNFORMATTED')
!IF(fv_out) OPEN(UNIT=19, FILE=file_fv, FORM = 'UNFORMATTED')
...
DO i_sim = 1, maxsteps
	...
	PRINT*,''
	WRITE(*,FMT='("Time step = ",i4,", dt = ",f4.1,", Completion = ",f6.1," %")') &
...
SUBROUTINE solve()
    ...
    !IF(i_sim .GT. 200) CALL inject_particles()   
    IF(i_sim .GT. 0)THEN !200) THEN
...
SUBROUTINE output_bin(ts)
    ...
    REAL*8 :: dv
    REAL*8 :: x, y, Bmag
    CHARACTER(LEN=40) :: filename1, filename2, filename3, filename4, filename_e, filename_i, &
                         filename_cur
    ...
    filename2 = trim('Frames/Snapshots2/fields_vercut_t=')//TRIM(uniti)//TRIM('.dat')
    filename3 = trim('Frames/Snapshots2/density_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename4 = trim('Frames/Snapshots2/B_xz_t=')//TRIM(uniti)//TRIM('.dat')
    filename_e = trim('Frames/Snapshots2/electrons_t=')//TRIM(uniti)//TRIM('.dat')
    filename_i = trim('Frames/Snapshots2/ions_t=')//TRIM(uniti)//TRIM('.dat')
        ...
        !END DO
         ...
         DO j = 1, nz
            DO i = 1, nx
                ...
                Bmag = SQRT(Bmag)
                WRITE(13) i*dx,j*dz,rho(i,ny/2,j),ne(i,ny/2,j),ni(i,ny/2,j)
    ...
    IF(particles_out) THEN
        ...
        OPEN(UNIT=9,FILE=filename_e, FORM = 'UNFORMATTED')
        IF(ns .EQ. 2) OPEN(UNIT=10,FILE=filename_i, FORM = 'UNFORMATTED')
        
        ! save particle states
        DO i = 1, Np_in(1)
            IF( ABS(particles(1,i)%y -ny/2) .LT. 25) WRITE(9) particles(1,i)%x,particles(1,i)%z
        END DO
       
        IF(ns .EQ. 2) THEN       
            DO i = 1, Np_in(2) 
                IF( ABS(particles(2,i)%y -ny/2) .LT. 25) WRITE(10) particles(2,i)%x,particles(2,i)%z
            END DO
        END IF
        
        CLOSE(UNIT=9)
        IF(ns .EQ. 2) CLOSE(UNIT=10)
