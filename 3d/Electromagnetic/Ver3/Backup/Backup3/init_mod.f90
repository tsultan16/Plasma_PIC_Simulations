...
SUBROUTINE init_particles()
	...
	INTEGER ::  i, j, k, l, counter, N_cell, i_v, imin, imax, jmin, jmax, kmin, kmax
	REAL*8 :: wp, qm, q, m, p, p1, p2, p3, gam


    ! set how many particles to put inside the computational domain 
    Np_in = 0
    ...
    DO i = 1, ns
		...
		q = SIGN(1.d0,qm) * ee !(wp**2)*eps0*(Lx*Ly*Lz/N)/qm
		m = q/qm
		
        PRINT*,'Species, q, m = ',i,q,m
    ...
    gam = 1.d0 / SQRT(1.d0 - (v0x**2+v0y**2+v0z**2)/c**2 ) 
    ...
    DO k = kmin, kmax
        DO j = jmin, jmax
            DO i = imin, imax
                ...
                ! set how many electron-ion pairs per cell
                N_cell = 1 
		
                !PRINT*,'Cell#, N_cell',i,j,N_cell
		
                IF(counter .GT. Np_in(1)) GO TO 111
                ...
                DO l = 1, N_cell	
                    ...
                    particles(1,counter)%x = i*dx + p1   !- 0.1*dx/N_cell   
                    particles(1,counter)%y = j*dy + p2   !+ l*dy/N_cell - 0.1*dy/N_cell   
                    particles(1,counter)%z = k*dz + p3
               
                    
                    ! set initial velocity : add three uniform random numbers between -0.5 and 0.5. This approximates a Maxwellian distribution.
                    CALL RANDOM_NUMBER(p1)                    
                    CALL RANDOM_NUMBER(p2)                    
                    CALL RANDOM_NUMBER(p3)                    
                    p1 = p1 - 0.5
                    p2 = p2 - 0.5
                    p3 = p3 - 0.5
                    particles(1,counter)%ux = gam * (v0x + vth_e*(vth_e + p1 + p2 + p3 - 1.5d0)) 
                    ...
                    p2 = p2 - 0.5
                    p3 = p3 - 0.5
                    particles(1,counter)%uy = gam * (v0y + vth_e*(p1 + p2 + p3 - 1.5d0))
                    ...
                    p2 = p2 - 0.5
                    p3 = p3 - 0.5                   
                    particles(1,counter)%uz = gam * (v0z + vth_e*(p1 + p2 + p3 - 1.5d0)) 
               
                    particles(1,counter)%oob = .FALSE.
                
                    IF(ns .EQ. 2) THEN
                        particles(2,counter)%x = particles(1,counter)%x  
                        particles(2,counter)%y = particles(1,counter)%y  
                        particles(2,counter)%z = particles(1,counter)%z
                        ...
                        p2 = p2 - 0.5
                        p3 = p3 - 0.5
                        particles(2,counter)%ux = gam * (v0x  + vth_i*(vth_i + p1 + p2 + p3 - 1.5d0)) 
                        ...
                        p2 = p2 - 0.5
                        p3 = p3 - 0.5
                        particles(2,counter)%uy = gam * (v0y + vth_i*(p1 + p2 + p3 - 1.5d0))
                        ...
                        p2 = p2 - 0.5
                        p3 = p3 - 0.5
                        particles(2,counter)%uz = gam * (v0z + vth_i*(p1 + p2 + p3 - 1.5d0))
                        
                        particles(2,counter)%oob = .FALSE.
    ...
    WRITE(*,FMT = '(" Inserted ",i4," particle pairs.")') counter-1
    ...
    IF (Np_in(1) .GT. 0) THEN
    
    particles(1,1)%x = nx/2    
    particles(1,1)%y = ny/2 
    particles(1,1)%z = nz/2
        
    particles(1,1)%ux = gam*v0x 
    particles(1,1)%uy = gam*v0y 
    particles(1,1)%uz = gam*v0z

    particles(1,counter)%oob = .FALSE.

       
    IF(ns .EQ. 2) THEN
        particles(2,1)%x = nx/2   
        particles(2,1)%y = ny/2   
        particles(2,1)%z = nz/2
        
        particles(2,1)%ux = gam*v0x 
        particles(2,1)%uy = gam*v0y 
        particles(2,1)%uz = gam*v0z
