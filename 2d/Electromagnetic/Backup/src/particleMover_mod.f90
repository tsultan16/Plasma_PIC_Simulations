...
SUBROUTINE move_particles(ts)
    ...
    ! Filter the current makes the spatial distriution smoother and attenuates short wavelength (high k) components of the fourier transform.
    ...
    ! Filtering will attentuate the aliased waves at higher frequencies and so mitagate this unphysical effect to a significant extent.
...
SUBROUTINE filter(f)
    ...
    REAL*8, INTENT(INOUT) ::  f(-1:nx+1,-1:ny+1)
