...
SUBROUTINE init_particles()
	...
	INTEGER ::  i, j, k, counter, N_cell, i_v
	REAL*8 :: wp, qm, q, m
    ...
    DO i = 1, nx
        DO j = 1, ny
            ...
            DO k = 1, N_cell	
                ...
                particles(1,counter)%x = (i-0.5d0)*dx + k*dx/N_cell - 0.1*dx/N_cell   
                particles(1,counter)%y = (j-0.5d0)*dy + 0.25*dy !+ k*dy/N_cell - 0.1*dy/N_cell   
               
                particles(2,counter)%x = (i-0.5d0)*dx + k*dx/N_cell - 0.1*dx/N_cell
                ...
                particles(1,counter)%vx = v0x !+ fv0(vmin+((vmax-vmin)/N_cell)*(k-0.5))
                particles(1,counter)%vy = v0y !+ fv0(vmin+((vmax-vmin)/N_cell)*(k-0.5))
             
                particles(2,counter)%vx = -v0x !+ fv0(vmin+((vmax-vmin)/N_cell)*(k-0.5)) 
			    particles(2,counter)%vy = v0y !+ fv0(vmin+((vmax-vmin)/N_cell)*(k-0.5))
	...
	END DO
            ...
            ! set initial position : x(0)		
            !particles(1,1)%x = xmin+0.25*Lx !(i-0.5d0)*dx + k*dx/N_cell - 0.1*dx/N_cell   
            !particles(1,1)%y = ymin+0.25*Ly  !(j-0.5d0)*dy + 0.25*dy !+ k*dy/N_cell - 0.1*dy/N_cell   
               
            !particles(2,1)%x = xmin+0.75*Lx !(i-0.5d0)*dx + k*dx/N_cell - 0.1*dx/N_cell
            !particles(2,1)%y = ymin+0.75*Ly  !(j-0.5d0)*dy + 0.25*dy !+ k*dy/N_cell - 0.1*dy/N_cell   
    ...
    DO i = 1, ns
		...
		DO  j = 1, N
			...
			IF(ABS(wc) .GT. 1.d-15) THEN	! w/ magnetic field 	
				! half acceleration
				particles(i,j)%vx = particles(i,j)%vx + a(1)*(-dt/4.d0)
				particles(i,j)%vy = particles(i,j)%vy + a(2)*(-dt/4.d0)
